...
namespace TOOLKIT_NAMESPACE
{
	...
	public:
		...
		friend std::ostream &operator<<(std::ostream &o, const RGBA &color);

		RGBA &hsl(float h, float s, float l);
		RGBA &hsv(float h, float s, float v);
		RGBA &rgb(int r, int g, int b);
	...
	public:
		...
		T a;
		GenericRGBA() {}
		GenericRGBA(const T &r, const T &g, const T &b) : r(r), g(g), b(b) {}
		GenericRGBA(const T &r, const T &g, const T &b, const T &a) : r(r), g(g), b(b), a(a) {}
	};
	...
	private:
		...
		GenericRGBA<float> current;
		GenericRGBA<float> step;
	...
	public:
		...
		void fadeInto(RGBA target, Uint16 frames = 8);
