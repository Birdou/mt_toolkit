...
int main(/*int argc, char *argv[]*/)
{
	...
	label.backgroundColor.hex(Black);
	...
	auto &bitmap = Mt_bitmap::create(app.window, 10, 250, 50, 50);
	for (size_t j = 0; j < bitmap.height(); ++j)
		for (size_t i = 0; i < bitmap.width(); ++i)
			bitmap.at(i, j).bw((i + j) * 2);

	auto &mainscrollArea = Mt_scrollarea::create(app.window, 10, 10, 500, 550, 0, 100);
	auto &scrollArea = Mt_scrollarea::create(app.window, 10, 10, 200, 500, 0, 100);
